/**
 * TopicSelectorDialog Modal
 * Dialog for managing topics assigned to a file using nice-modal
 */
	...
	({ filePath, fileName, initialTopicIds }) => {
		...
		const handleSave = async () => {
			try {
				...
				modal.resolve(selectedTopicIds);
				modal.hide();
										...
										<div className="p-2 space-y-1">
											{filteredTopics.length === 0 ? (
												...
												filteredTopics.map((topic) => {
													const isSelected = selectedTopicIds.includes(
														topic.id,
													);
