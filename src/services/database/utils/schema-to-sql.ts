...
function getTableName(table: PgTable): string {
	return (table as unknown as Record<string | symbol, unknown>)[
	...
	try {
		...
		for (const [fieldName, column] of Object.entries(columns)) {
			...
			if (hasDefaultRandom && columnType === "PgUUID") {
				...
				isAutoIncrement &&
				(columnType === "PgInteger" || columnType === "PgSerial")
			) {
			...
			if (col.references && typeof col.references === "function") {
				try {
					...
					const referencedTable = getTableName(
						referencedColumn.table as PgTable,
					);
					const referencedColumnName = referencedColumn.name as string;
					if (referencedTable && referencedColumnName) {
						foreignKeys.push(
							`FOREIGN KEY (${columnName}) REFERENCES ${referencedTable}(${referencedColumnName})`,
						);
					}
				} catch {
		...
		return indexes
			.map((index) => {
				try {
					...
					const config = indexData.config as
						| Record<string, unknown>
						| undefined;
					const indexName =
						(config?.name as string) || `${tableName}_${Date.now()}_idx`;
...
export function toFullTableSQL(table: PgTable): {
	table: string;
	indexes: string[];
} {
	return {
