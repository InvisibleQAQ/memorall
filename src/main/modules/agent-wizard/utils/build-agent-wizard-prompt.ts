...
5. Expected response output — required. Decide whether the agent should primarily answer in markdown or produce visual/artifact output. If unclear and the choice materially changes the agent, ask the user whether they expect markdown or visual output.
6. Skills — add when relevant to the user's requested workflows. If visual output is expected, choose the relevant style-specific design skill from the default skills catalog when available, such as dashboard, application, shadcn, clean, bento, editorial, brutalism, or another matching awesome-design-skills mode.
...
- When the user asks for visual presentation, ask or infer the design direction they want to see, then add the matching style-specific awesome-design-skills skill when available before presenting the result with an artifact.
...
8. Expected Output & Design Direction — state whether the agent should produce markdown or visual/artifact output. For visual output, state what design style the user wants to see or how the agent should choose an appropriate design skill/mode.
...
- For any visual/artifact agent, include the expected output type and design direction in the instruction. If the design direction is not explicit, tell the agent to ask what style the user wants to see or infer and enable a suitable style-specific design skill when the request is already clear enough.
