...
- A complete agent draft must decide the expected output mode. For markdown-first agents, specify the markdown structure in the instruction. For visual-first agents, enable artifact-capable features when available, select exactly one style-specific design skill when present (do not add more than one), and ask what design style the user wants to see if the visual direction is materially unclear.
...
- When the user asks for visual presentation, ask or infer the design direction they want to see, then add exactly one matching style-specific awesome-design-skills skill when available before presenting the result with an artifact. Do not add more than one design style skill — pick the best fit and follow it.
