...
const extractWebAccessPayload = (item: ActionItem): WebAccessPayload | null => {
...
const WebAccessPreview: React.FC<{ payload: WebAccessPayload }> = ({
	...
	return (
		<div className="w-full rounded-lg border border-border/60 overflow-hidden bg-background">
			...
			{canFrameUrl ? (
				<iframe
					title={`Web access preview: ${payload.url}`}
					src={previewUrl}
				...
				<div className="px-3 py-4 text-sm text-muted-foreground">
					No renderable URL/HTML found for web preview.
				</div>
			)}
			{htmlPreview ? (
